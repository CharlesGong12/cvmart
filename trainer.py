...
class BaseTrainer:
    ...
    def _setup_train(self, world_size):
        ...
        self.scaler1 = amp.GradScaler(2**8,enabled=self.amp)
        self.scaler2 = amp.GradScaler(2**8,enabled=self.amp)
        if world_size > 1:
    ...
    def _do_train(self, world_size=1):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            for i, batch in pbar:
                ...
                # Backward
                self.scaler1.scale(self.loss).backward()
    ...
    def optimizer_step(self, batch):
        """Perform a single step of the training optimizer with gradient clipping and EMA update."""
        self.scaler1.unscale_(self.optimizer)
        ...
        self.scaler2.unscale_(self.optimizer)
        torch.nn.utils.clip_grad_norm_(self.model.parameters(), max_norm=10.0)
        if sam_first_step_applied:
            self.optimizer.second_step()

        self.scaler2.step(self.optimizer)
        self.scaler2.update()
        self.optimizer.zero_grad()
...
class SAM(torch.optim.Optimizer):
    def __init__(self, params, base_optimizer, rho=1, adaptive=True, **kwargs):
    ...
    def first_step(self, zero_grad=False):
        # with torch.cuda.amp.autocast():
            ...
            # print(f'grad norm of first step: {grad_norm}')
    ...
    def second_step(self, zero_grad=False):
        # with torch.cuda.amp.autocast():
            grad_norm = self._grad_norm()
            # print(f'grad norm of second step: {grad_norm}')
            for group in self.param_groups:
                for p in group["params"]:
                    if p.grad is None: continue
